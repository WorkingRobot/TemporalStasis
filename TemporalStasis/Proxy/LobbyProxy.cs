...
public class LobbyProxy(
    ...
    public event IProxy.RawFrameInterceptor? OnRawServerboundFrame;
    public event IProxy.RawFrameInterceptor? OnRawClientboundFrame;
    ...
    public async Task SendIpcPacketAsync(int id, IpcInterceptedPacket packet, bool serverbound) {
        if (this.clients.TryGetValue(id, out var client))
            await client.SendPacketAsync(packet.ToRawPacket(), serverbound);
    }
    ...
    private async Task HandleClient(TcpClient client) {
        ...
        var proxyClient = new LobbyProxyClient(
            ...
            (frame, serverbound) => {
                try {
                    if (serverbound) {
                        this.OnRawServerboundFrame?.Invoke(frame, ConnectionType.Lobby);
                    } else {
                        this.OnRawClientboundFrame?.Invoke(frame, ConnectionType.Lobby);
                    }
                } catch (Exception e) {
                    Console.WriteLine(e);
                }
